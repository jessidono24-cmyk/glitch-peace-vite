# Task PLAYTEST2 — Deep Systematic Test: All Modes × Dreamscapes × Cosmologies

## Goal
Run a comprehensive automated test through every combination of game mode,
dreamscape, and cosmology. Log every crash, freeze, visual bug, and error.
Then fix each issue found. Repeat the cycle until all combinations pass.

This task is designed to be run REPEATEDLY. Each run produces a report.
The agent fixes issues found, then runs again. Cycle continues until
the report shows 0 failures.

## Definition of Done
- [ ] All 10 game modes load without crash
- [ ] All 10 modes play for at least 10 seconds without freezing
- [ ] All 18 dreamscapes are selectable and load
- [ ] All 13 cosmologies are selectable
- [ ] No mode shows grid tiles unless it IS the grid mode
- [ ] No console errors on any mode/dreamscape combination
- [ ] No [object Object] on any screen
- [ ] Canvas fills full screen in every mode
- [ ] PLAYTEST2_REPORT.md shows 0 failures

---

## Phase 1: Create the Deep Test File

Create `tests/playtest-deep.js`:
...
test('GLITCH·PEACE Deep Integration Test', async () => {
...

  {
    ...
    if (titleText.includes('GRID-CLASSIC')) {
      fail('Title screen: GRID-CLASSIC label removed', 'Label still present');
    } else {
...
## Phase 2: Update playwright.config.js

Make sure `playtest-deep.js` is included in test discovery:
...
After each test run, do this:

1. Read `PLAYTEST2_REPORT.md`
2. For each ❌ AUTOMATED FAILURE:
   - Open the screenshot
   - Find the root cause in the relevant source file
   - Fix it
   - Do NOT move to the next failure until the current one is fixed
3. Run `npm run build` — must pass before re-running tests
4. Run the test again
5. Repeat until report shows `Automated failures: 0`

### Common fixes by failure type:

**"CRASH" failures:**
- Open the mode file that crashed
- Find the error in the screenshot/console log
- Fix the specific line

**"frozen" failures:**
- Check the mode's update() function for infinite loops
- Add `if (dt > 0.5) return;` safety guard at top of update()
- Check for missing break statements in switch blocks

**"Canvas too narrow" failures:**
- This means UI1 didn't fully apply to this mode
- Find where this mode sets its rendering area
- Replace hardcoded widths with canvas.width * 0.95

**"[object Object]" failures:**
- Find every HUD text render in renderer.js
- Wrap in: `String(value?.name || value?.label || value || '')`

**"console errors" failures:**
- Read the error message in the screenshot
- Find the file and line referenced
- Fix the import or undefined reference

---

## Phase 4: Final verification

When `PLAYTEST2_REPORT.md` shows `Automated failures: 0`:
1. Open each visual check screenshot
2. Confirm no grid tiles in non-grid modes
3. Confirm canvas fills full screen
4. Commit with message: `test: PLAYTEST2 all modes/dreamscapes/cosmologies verified`

## Commit message (after all fixes)
```
fix: PLAYTEST2 cycle complete -- all modes, dreamscapes, cosmologies passing
```
