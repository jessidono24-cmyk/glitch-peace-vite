# Task PLAYTEST4 — Combinatorial Deep Test: Modes × Dreamscapes × Cosmologies + Level Completion

## Goal
Test every game mode with every dreamscape, verify visual differentiation
between modes, confirm playstyles work as pause-menu mood modifiers,
attempt to complete 1-2 levels in each mode, and verify research-mapped
mechanics are present and functioning.

This is the most comprehensive test task. It replaces manual playtesting.
The agent should give itself as much time as needed — no timeout limit.

## Definition of Done
- [ ] Every mode tested with at least 3 different dreamscapes
- [ ] Every mode has visually distinct appearance (no grid tiles in non-grid modes)
- [ ] At least 1 level completed in each mode
- [ ] Playstyle selection appears in PAUSE menu (not pre-game)
- [ ] Opening sequence: black screen → "consciousness simulation loading" → menu
- [ ] Canvas is full screen in every single mode and menu
- [ ] Playwright timeout set to 3,600,000ms (1 hour) — never cut off
- [ ] PLAYTEST4_REPORT.md written with PASS/FAIL for every combination tested
- [ ] Zero automated failures in final report
...
export default defineConfig({
  ...
  timeout: 3_600_000,  // 1 hour — never cut off a test
  retries: 0,
  use: {
...
## Create `tests/playtest-combo.js`
...
## After PLAYTEST4 runs:

1. Read `PLAYTEST4_REPORT.md`
2. Fix every automated failure (same loop as PLAYTEST3)
3. Open the visual check screenshots manually and note which visual checks fail
4. Write a `task-VIS1.md` for each visual failure that requires code work
5. Rerun until automated failures = 0

## Commit message
```
test: PLAYTEST4 combinatorial test -- all modes x dreamscapes x cosmologies
```
