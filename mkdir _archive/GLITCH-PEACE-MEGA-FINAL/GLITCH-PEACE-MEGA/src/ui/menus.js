...
export class MenuSystem {
  constructor({ CFG, onStartNew, onContinue, onQuitToTitle, onRestart }) {
    this.CFG = CFG;
    ...
    this.screen = 'title'; // 'title' | 'pause' | 'options' | 'tutorial' | 'credits'
    this.sel = 0;
    this.tutPage = 0;
  ...
  handleKey(e) {
    ...
    // Global escapes
    if (k === 'Escape') {
      if (this.screen === 'options' || this.screen === 'tutorial' || this.screen === 'credits') {
        // go back to title or pause (depends where you came from; simplest: title)
        this.open('title');
  ...
  _handleTutorial(k) {
    ...
    if (k === 'ArrowRight' || k === 'd' || k === 'D') {
      this.tutPage = Math.min(this.getTutorialPages().length - 1, this.tutPage + 1);
      return { consumed: true };
    }
    if (k === 'Enter' || k === ' ') {
      // Enter = next page, last page returns to title
      const last = this.getTutorialPages().length - 1;
  ...
  getItems() {
    ...
    common.push({
      label: isPause ? 'QUIT TO TITLE' : 'EXIT (CLOSE TAB)',
      action: () => this.onQuitToTitle({ to: 'title' }),
    });

    // Filter out disabled from selection navigation *visually* but keep them selectable? better: keep, but block action.
    return common;
  }
  ...
  getOptionRows() {
    ...
    return [
      ...
      },
      {
        ...
        value: cfg.stillnessMode ? 'ON' : 'OFF',
        toggle: () => (cfg.stillnessMode = !cfg.stillnessMode),
      },
      {
        label: 'INTENSITY',
        value: cfg.intensityMul.toFixed(2),
  ...
  getTutorialPages() {
    // “Thorough + concise + mystery”: show the rules clearly, hint at deeper layers.
    return [
      {
        title: 'TUTORIAL 1/4 · CONTROLS',
        lines: [
          'Move: WASD / Arrow Keys',
          'Pause Menu: ESC',
          'Goal: collect all ◈ Peace tiles',
          '',
          'Tip: move slowly. predict. breathe.',
        ],
      },
      {
        title: 'TUTORIAL 2/4 · TILES',
        lines: [
          '◈ Peace: heal + score',
          '◆ Insight: tokens (future upgrades)',
          'Red hazards: damage',
          'Walls: block movement',
          '',
          'The grid is a matrix: it reacts to patterns.',
        ],
      },
      {
        title: 'TUTORIAL 3/4 · EMOTION FIELD',
        lines: [
          'Your moves shift an invisible emotional field.',
          'Higher distortion makes the world feel “louder.”',
          '',
          'You can still win inside distortion.',
          'The trick: small stable actions.',
        ],
      },
      {
        title: 'TUTORIAL 4/4 · DREAMSCAPES (TEASER)',
        lines: [
          'Later builds add dreamscapes as “biomes”:',
          'Maze · Bog · Leaping Field · Lake/Lodge · Summit',
          '',
          'Each dreamscape has its own rules…',
          'but the win condition stays simple.',
          '',
          'ENTER: finish tutorial',
        ],
      },
    ];
  }
  ...
  _drawList(ctx, w, h) {
    const isPause = this.screen === 'pause';
    this._drawHeader(ctx, w, h, isPause ? 'PAUSED' : 'v5 · pattern recognition engine');
    ...
    ctx.font = '8px Courier New';
    ctx.textAlign = 'center';
    ctx.fillText('↑/↓ to select · ENTER to confirm · ESC to pause/exit', w / 2, by + boxH + 24);
    ctx.textAlign = 'left';
  }
  ...
  _drawOptions(ctx, w, h) {
    ...
    ctx.font = '8px Courier New';
    ctx.textAlign = 'center';
    ctx.fillText('↑/↓ select · ←/→ adjust · ENTER toggle · BACKSPACE back', w / 2, by + boxH + 24);
    ctx.textAlign = 'left';
  }
  ...
  _drawTutorial(ctx, w, h) {
    ...
    const pages = this.getTutorialPages();
    ...
    ctx.fillStyle = '#131328';
    ctx.font = '8px Courier New';
    ctx.fillText('←/→ page · ENTER next · BACKSPACE back', w / 2, by + boxH - 22);
  ...
  _drawCredits(ctx, w, h) {
    ...
    ctx.fillStyle = '#131328';
    ctx.font = '8px Courier New';
    ctx.fillText('ENTER to return', w / 2, h / 2 + 28);
    ctx.textAlign = 'left';
  }
}