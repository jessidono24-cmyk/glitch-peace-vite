...
const STRATEGY_TIPS = [
  ...
  { level: 25, tip: 'Control zones + Matrix B is a powerful defensive combination.' },
  { level: 40, tip: 'Watch the purgDepth bar — a full bar signals compounding hazards ahead.' },
...
export class StrategicThinking {
  constructor() {
    ...
    this._contZones        = 0;  // containment zones placed
  ...
  onContainmentZone() { this._contZones++; }
  ...
  get strategicScore() {
    ...
    const matrixBalance = totalDamage > 0
      ...
      : 0.6;
    const toolScore     = Math.min(1, (this._contZones * 12 + this._freezeUses * 8 + this._impulseCancel * 5) / 100);
  ...
  resetSession() {
    ...
    this._tokensSpentValue = 0; this._contZones = 0;
    this._freezeUses = 0; this._impulseCancel = 0;
  }
