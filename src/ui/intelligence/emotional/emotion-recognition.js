...
const BENEFITS_FROM_COHERENCE = new Set(['fear', 'anxiety', 'sadness', 'shame', 'disgust', 'anger']);
...
export class EmotionRecognition {
  ...
  observe(emotionId, emotionValue, matrixActive) {
    if (emotionValue < LABEL_THRESHOLD) { this._lastLabelledId = null; return; }
    ...
    if (emotionId !== this._lastLabelledId) {
      ...
      this._labelledCount++;
      this._flashLabel = { id: emotionId, ...def, timer: 90 };
