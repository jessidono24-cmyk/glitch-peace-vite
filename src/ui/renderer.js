...
export function drawGame(ctx, ts, game, matrixActive, backgroundStars, visions, hallucinations, anomalyActive, anomalyData, glitchFrames, DPR, ghostPath) {
  ...
  for (const v of visions) {
    ...
    ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center'; ctx.fillText(v.text, v.x, v.y);
  }
  ...
  if (ISO) for (let li = 0; li < sz * sz; li++) {
    ...
    if (td.sym && val !== T.VOID && val !== T.WALL && val !== T.HIDDEN) {
      ctx.fillStyle = tp.bd; ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
      ctx.globalAlpha = 0.7;
  ...
  for (let y = 0; y < sz; y++) {
    for (let x = 0; x < sz; x++) {
      ...
      if (val === T.ARCHETYPE) {
        ...
        ctx.font = fs(20, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
      ...
      if (td.sym && val !== T.VOID && val !== T.WALL && val !== T.PEACE && val !== T.INSIGHT &&
        ...
        ctx.fillStyle = tp.bd; ctx.font = fs(16, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ...
  if (ghostPath && ghostPath.length > 0) {
    ghostPath.forEach((step, i) => {
      ...
      if (step.hpDelta !== 0) {
        ctx.fillStyle = col; ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center'; ctx.globalAlpha = Math.max(0.1, alpha + 0.1);
  ...
  if (g.contZones) {
    ...
    for (const cz of g.contZones) {
      ...
      ctx.shadowBlur = 0;
      // Label
      ctx.fillStyle = `rgba(0,200,160,${czAlpha})`; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ...
  for (const h of hallucinations) {
    ...
    ctx.fillStyle = 'rgba(170,0,255,0.6)'; ctx.font = fs(20, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ...
  if (g.boss && g.boss.hp > 0) {
    ...
    ctx.font = 'bold '+ fs(13, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
...
function drawTutorialHint(ctx, w) {
  ...
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#00cc55';
...
function drawEmergenceFlash(ctx, w, h) {
  ...
  ctx.textAlign = 'center';
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#ffaa44';
  ...
  ctx.shadowBlur = 0;
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#886633';
...
function drawEmotionFlash(ctx, w, h) {
  ...
  ctx.globalAlpha = alpha;
  ctx.textAlign = 'center';
  ctx.font = 'bold '+ fs(20, ctx.canvas) + 'px Courier New';
  ...
  if (flash.tip || flash.desc) {
    ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#aaaaaa';
...
function drawRealityCheck(ctx, w, h) {
  ...
  ctx.textAlign = 'center';
  ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#cc88ff';
  ctx.fillText('REALITY CHECK', w / 2, y + 14);
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#aaaacc';
  ctx.fillText(dy.rcPrompt.q, w / 2, y + 28);
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#556655';
  ctx.fillText(dy.rcPrompt.hint, w / 2, y + 41);
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#8844ff';
...
function drawEmotionRow(ctx, w, efData) {
  ...
  // Bar labels
  ctx.font = fs(11, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#223344';
...
function drawHUD(ctx, g, w, h, gp, sx, sy, matrixActive) {
  ...
  // HP
  const hpBarW = 138;
  ctx.fillStyle = '#333'; ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.fillText('HP', 14, 30);
  ...
  ctx.fillStyle = '#444'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText(g.hp + '/' + UPG_ref.maxHp, 32 + hpBarW + 4, 30);

  // Energy
  const eBarW = 138;
  ctx.fillStyle = '#222'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('EN', 14, 50);
  ...
  // PurgDepth bar (dark red depth indicator)
  {
    ...
    if (pd > 0.05) {
      ctx.fillStyle = pd > 0.7 ? '#ff2222' : '#660000'; ctx.font = fs(11, ctx.canvas) + 'px Courier New';
  ...
  if (UPG_ref.glitchPulse) {
    ...
    ctx.fillStyle = pChr >= 1 ? '#ff00ff' : '#553355'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('PULSE' + (pChr >= 1 ? ' READY' : ''), 32 + eBarW + 4, 65);
  }
  ...
  if (UPG_ref.freeze) {
    ...
    if (fTimer > 0) {
      ...
      ctx.fillStyle = '#0088ff'; ctx.font = fs(11, ctx.canvas) + 'px Courier New';
      ctx.fillText('FREEZE ' + Math.ceil(fTimer / 1000) + 's', 32 + eBarW + 4, 63 + (UPG_ref.glitchPulse ? 7 : 0));
    } else {
      ctx.fillStyle = '#334455'; ctx.font = fs(11, ctx.canvas) + 'px Courier New';
  ...
  // ImpulseBuffer hold progress (shown when holding into hazard)
  {
    ...
    if (prog > 0 && prog < 1) {
      ...
      ctx.fillStyle = '#ffaa00'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (g.archetypeActive && g.archetypeType) {
    ...
    ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText(arch ? arch.name + ' ACTIVE' : '[ARCH ACTIVE]', 14, 90); ctx.shadowBlur = 0;
  } else if (UPG_ref.shield && UPG_ref.shieldTimer > 0) {
    ctx.fillStyle = '#00ffff'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('SHIELD×' + UPG_ref.shieldTimer, 14, 90);
  } else if (UPG_ref.shieldCount > 0) {
    ctx.fillStyle = '#334455'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('streak ' + UPG_ref.shieldCount + '/3', 14, 90);
  }

  if (UPG_ref.comboCount > 1) {
    ctx.fillStyle = '#ffcc00'; ctx.shadowColor = '#ffcc00'; ctx.shadowBlur = 6;
    ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('COMBO ×' + UPG_ref.resonanceMultiplier.toFixed(1), 14, 105);
    ctx.shadowBlur = 0;
  }

  ctx.fillStyle = '#00eeff'; ctx.shadowColor = '#00eeff'; ctx.shadowBlur = 5;
  ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.fillText('◆×' + (window._insightTokens || 0), 14, 113); ctx.shadowBlur = 0;

  // Alchemy seeds
  const seedsDisplay = window._alchemySystem?.seedsDisplay;
  if (seedsDisplay) {
    ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#cc88ff';
  ...
  if (luc > 0) {
    ...
    ctx.font = fs(11, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#8844ff';
    ctx.fillText('LUC', lx + lw + 3, ly + 4);
  }

  // Score
  ctx.fillStyle = '#00ff88'; ctx.shadowColor = '#00ff88'; ctx.shadowBlur = 10;
  ctx.font = 'bold '+ fs(24, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ctx.fillText(String(g.score).padStart(7, '0'), w / 2, 38); ctx.shadowBlur = 0;
  ctx.fillStyle = '#222838'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText('SCORE', w / 2, 52);

  const mC = matrixActive === 'A' ? '#ff0055' : '#00aa55';
  ctx.fillStyle = mC; ctx.shadowColor = mC; ctx.shadowBlur = 7;
  ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New'; ctx.fillText('MTX·' + matrixActive, w / 2 - 16, 68); ctx.shadowBlur = 0;

  ctx.textAlign = 'right';
  ctx.fillStyle = '#445566'; ctx.font = fs(14, ctx.canvas) + 'px Courier New'; ctx.fillText('LVL ' + g.level, w - 12, 30);
  ...
  if (cs) {
    ...
    if (cs.levelUpMsg) {
      ...
      ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillText(cs.levelUpMsg, w - 12, 114);
      ctx.shadowBlur = 0;
    }
  }
  ctx.textAlign = 'left';
  ...
  if (vocabWord) {
    ...
    if (!showTarget) {
      ...
      ctx.fillStyle = '#886644'; ctx.font = fs(14, ctx.canvas) + 'px Courier New';
  ...
  if (sigil && (window._sigilAlpha || 0) > 0) {
    ...
    ctx.font = 'bold '+ fs(20, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
    ctx.fillText(sigil.symbol, sgX + 20, sgY + 24); ctx.shadowBlur = 0;
    ctx.fillStyle = '#ffcc66'; ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New';
    ctx.fillText(sigil.tradition.split('(')[0].trim().slice(0, 22), sgX + 80, sgY + 15);
    ctx.fillStyle = '#aa9966'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
    const meanParts = sigil.meaning.split('·');
    ctx.fillText((meanParts[0] || '').trim(), sgX + 80, sgY + 28);
    ctx.fillStyle = '#664422'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
    ctx.fillText((meanParts[1] || meanParts[0] || '').trim(), sgX + 80, sgY + 40);
    ctx.fillStyle = '#443322'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (ad && ad.text && ad.alpha > 0.02) {
    ...
    ctx.fillStyle = '#ddeedd'; ctx.font = 'italic '+ fs(14, ctx.canvas) + 'px Courier New';
  ...
  if (banner) {
    ...
    ctx.fillStyle = '#ccbbaa'; ctx.font = fs(14, ctx.canvas) + 'px Courier New';
    ctx.fillText(banner.description, w/2, h/2 + 4);
    ctx.fillStyle = '#665544'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (bpb && bpb.alpha > 0.02) {
    ...
    ctx.fillStyle = '#aa8888'; ctx.font = 'italic '+ fs(14, ctx.canvas) + 'px Courier New';
    ctx.fillText('"' + (bpb.quote || '') + '"', w/2, h * 0.14 + 38);
    ctx.fillStyle = '#553333'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (qf && qf.alpha > 0.02) {
    ...
    ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
    ctx.fillText(qf.emoji + '  QUEST COMPLETE  ' + qf.emoji, w/2, h * 0.82 + 16); ctx.shadowBlur = 0;
    ctx.fillStyle = '#ddeedd'; ctx.font = fs(14, ctx.canvas) + 'px Courier New';
    ctx.fillText(qf.text, w/2, h * 0.82 + 32);
    ctx.fillStyle = '#556644'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (af && af.alpha > 0.02) {
    ...
    ctx.font = 'bold '+ (af.stone ? fs(16, ctx.canvas) : fs(14, ctx.canvas)) + 'px Courier New'; ctx.textAlign = 'center';
    ctx.fillText(af.text, w/2, h * 0.55 + 18); ctx.shadowBlur = 0;
    ctx.fillStyle = '#998877'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (alch && alch.active && (alch.seedsDisplay || alch.phase)) {
    ...
    ctx.fillStyle = '#cc88ff'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ...
  if (pml) {
    ...
    ctx.fillStyle = '#00aa55'; ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
  ...
  if (em && em.flash) {
    ...
    ctx.fillStyle = '#667799'; ctx.font = fs(13, ctx.canvas) + 'px Courier New';
  ...
  if (ck && ck.flash) {
    ...
    ctx.fillStyle = '#aa8866'; ctx.font = fs(14, ctx.canvas) + 'px Courier New';
  ...
  if (iqData) {
    ...
    if (eqfl && iqData.eqFlashAlpha > 0) {
      ...
      ctx.fillStyle = eqfl.color; ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New';
    ...
    if (challenge && iqData.challengeAlpha > 0) {
      ...
      ctx.fillStyle = '#aaccee'; ctx.font = fs(20, ctx.canvas) + 'px Courier New';
      ctx.fillText(challenge.seq.join('  ·  ') + '  ·  ?', w/2, h*0.42 + 38);
      ctx.fillStyle = '#44aa66'; ctx.font = 'bold '+ fs(16, ctx.canvas) + 'px Courier New';
    ...
    if (tut && tut.text) {
      ...
      ctx.shadowBlur = 0;
      ctx.fillStyle = '#ccffcc'; ctx.font = fs(14, ctx.canvas) + 'px Courier New';
    ...
    if (srt !== undefined && srt !== null && srt >= 0) {
      ...
      ctx.fillStyle = timerColor; ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
    ...
    if (mr !== undefined && mr !== null) {
      ...
      ctx.fillStyle = mrColor; ctx.font = 'bold '+ fs(14, ctx.canvas) + 'px Courier New'; ctx.textAlign = 'center';
...
function drawDashboard(ctx, w, h) {
  ...
  // Title
  ctx.font = 'bold '+ fs(16, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#00ff88';
  ctx.textAlign = 'center';
  ctx.fillText('INTEGRATION DASHBOARD', w / 2, 34);
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#336633';
  ...
  // Intelligence scores
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#225522';
  ...
  // Footer hint
  ctx.font = fs(13, ctx.canvas) + 'px Courier New'; ctx.fillStyle = '#1a2a1a'; ctx.textAlign = 'center';
