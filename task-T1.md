...
## Goal
Build the temporal system: 8 lunar phases + 7 weekly planetary harmonics,
each with modifiers that will scale enemy speed, insight token value,
and emotion decay. T1 is a self-contained new file only — nothing is
wired into main.js yet (that is T2).

## Definition of Done
- [ ] `npm run build` passes clean
- [ ] File exists at `src/systems/temporal-system.js`
- [ ] `window._temporalSystem` is accessible in browser console
      (wired temporarily in T2 — for now just confirm the export works)
- [ ] No changes to any existing file

## Scope — create ONE new file only
- `src/systems/temporal-system.js`  (new)

Do NOT touch main.js, renderer.js, state.js, or any other existing file.

---

## Full file to create

Create `src/systems/temporal-system.js` with exactly this content:
...
## Verification
```bash
npm run build   # must pass clean, no errors
```

The file should compile without warnings. No browser test needed yet --
T2 will wire it in and expose `window._temporalSystem`.

## Commit message
```
feat: T1 temporal system -- lunar phases and planetary days
```
